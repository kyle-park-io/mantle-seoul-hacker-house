...
### 1) 레퍼런스 확보 (`contracts/`)
배포된 Aave 컨트랙트의 주소·ABI·소스를 `contracts/aave-v3-mantle/` 에 먼저 가져왔다. 주소는 권위 있는 [aave-address-book](https://github.com/bgd-labs/aave-address-book) 에서. 이 단계에서 쓰는 컨트랙트:
...
> 정답지는 **Foundry 로만** 제공한다. 같은 조회를 Hardhat(`network.create({network:"mantleFork"})` + viem `readContract`)으로도 만들 수 있으며, 실습에서는 둘 중 어느 쪽이든 OK — 같은 주소를 읽으니 결과가 일치한다. 두 스택 패턴은 `CLAUDE.md` 의 "fork 테스트 작성 패턴" 참고.
